...
require 'kitchen/driver/zone_helpers'
require 'net/ssh'
...
module Kitchen
  module Driver
    ...
    include Kitchen::Driver::ZoneHelpers
      ...
      default_kitchen_username = 'kitchen'
      default_config :kitchen_username, default_kitchen_username
      # The public and private halves of the key pair we'll use to authenticate
      # when SSH'ing into the zone.
      default_config :ssh_public_key,  File.join(Dir.pwd, ".#{default_kitchen_username}", 'id_rsa.pub')
      default_config :ssh_private_key, File.join(Dir.pwd, ".#{default_kitchen_username}", 'id_rsa')
      ...
      def generate_keypair
        ...
        unless keypair?
          SSH_KEY_MUTEX.synchronize do
            ...
            public_key = "ssh-rsa #{ssh2_key_blob} #{config[:kitchen_username]}@#{config[:zone_name]}"
      ...
      def create_zone(state)
        ...
        zone_cfg_path_local = File.join(Dir.pwd, ".#{config[:kitchen_username]}", zone_config)
        ...
        zone_sc_profile_args = {
          zone_name:         zone_name,
          kitchen_username: config[:kitchen_username],
          ssh_public_key:    IO.read(config[:ssh_public_key])
        }
        zone_profile = "#{zone_name}_profile.xml"
        zone_profile_path_local = File.join(Dir.pwd, ".#{config[:kitchen_username]}", zone_profile)
